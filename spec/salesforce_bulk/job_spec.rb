...
RSpec.describe SalesforceBulk::Job do
  ...
  let (:batch_ids) { ["some-batch-id-1", "some-batch-id-2"] }
  ...
  let (:connection) {
    ...
    XML
      ...
      allow_any_instance_of(SalesforceBulk::Connection).to receive(:post_xml).with("test.salesforce.com", "/services/Soap/u/" + version, request_body, {'Content-Type' => 'text/xml; charset=utf-8', 'SOAPAction' => 'login'}).and_return(response_body)
 ...
 <apexProcessingTime>0</apexProcessingTime>
    ...
    it 'can adhere to the contract with Salesforce' do
      ...
      expect(sf_bulk_job.class.class_variable_get(:@@job_id)).to eq(job_id)
...
<?xml version="1.0" encoding="UTF-8"?><batchInfo
 ...
 <apexProcessingTime>0</apexProcessingTime>
    ...
    before do
      ...
      allow(connection).to receive(:post_xml).with(nil, "job/#{job_id}/batch/", request_body, xml_headers).and_return(response_body)
    end

    it 'can adhere to the contract with Salesforce' do
      expect(sf_bulk_job.add_query).to eq(batch_id)
      expect(sf_bulk_job.class.class_variable_get(:@@batch_id)).to eq(batch_id)
    end
  end
 ...
 <batchInfo>
  ...
  <numberRecordsProcessed>800</numberRecordsProcessed>
    ...
    before do
      ...
      allow(connection).to receive(:get_request).with(nil, "job/#{job_id}/batch", Hash.new).and_return(response_body)
    end
...
'<?xml version="1.0" encoding="UTF-8"?><batchInfo
 ...
 <apexProcessingTime>0</apexProcessingTime>
    ...
    before do
      ...
      allow(connection).to receive(:get_request).with(nil, "job/#{job_id}/batch/#{batch_id}", Hash.new).and_return(response_body)
    end
...
"00QccsalesforceID2","C234567","005PothersalesforceID2","false"
    ...
    before do
      ...
      allow(connection).to receive(:get_request).with(nil, "job/#{job_id}/batch/#{batch_id}/result", xml_headers).and_return(response_body_result_list)
      allow(connection).to receive(:get_request).with(nil, "job/#{job_id}/batch/#{batch_id}/result/#{result_id}", xml_headers).and_return(response_body_result_item_info)
    end
